...
def login():
    ...
    if not auth or not auth.username or not auth.password:
        return make_response('Login ivalido', 401, {'WWW-Authenticate': 'Basic realm="Login obrigatório"'})
...
def obter_autor_por_id(autor,id_autor):
    ...
    autor_atual["nome"] = autor.nome
    autor_atual["email"] = autor.email
    return jsonify(f'Voçe buscou pelo autor: {autor_atual}')
...
def alterar_autor(autor,id_autor):
    ...
    autor = Autor.query.filter_by(id_autor=id_autor).first()
    if not autor:
        return jsonify({'Mensagem':'Este ususario não foi encontrado!'})
...
def excluir_autor(autor,id_autor):
    ...
    return jsonify({'Mensagem':'Autor Excluido!'})
